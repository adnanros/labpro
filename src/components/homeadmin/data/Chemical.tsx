...
import { getChemical, listChemicals,  } from '../../../graphql/queries';
...
class Chemical extends Component<any,IState> {
    ...
    componentDidMount(){
      this.props.getDataList(listChemicals);
    }
    render() {
      return (
        <div>
          {this.props.isLoadingFailed && <CButton onClick={()=>{this.props.getDataList(listChemicals)}}>Refresh</CButton>}
                ...
                <CTableBody>
                  {
                    ...
                    this.props.data.map((item: any, index:any) => (
                      <CTableRow key={index}>
                        ...
                        <CTableDataCell>
                        <div>
                          <CButton className='primary' color="link" onClick={()=>{this.setState({showDetail: true}); this.props.getItemDetail(getChemical,item.id);}} disabled={this.props.isLoaingItemDetail}>Detail</CButton>
            ...
            >
              ...
              <CModalFooter>
                ...
                <CButton onClick={()=>{
                  this.props.deleteItem(deleteChemical,this.state.toBeDeletedId,listChemicals); this.setState({showDeleteAlert: false})} } color="primary">Ok</CButton>
              </CModalFooter>
            </CModal>
            ...
            >
              <CModalHeader>
                <CModalTitle>Register A Chemical</CModalTitle>
              </CModalHeader>
              <CModalBody>
                <CreateChemicalComponent onclick={()=> this.setState({showCreate: false})}/>
              </CModalBody>
            </CModal>
            ...
            >
              <CModalHeader>
                <CModalTitle>Edit A Chemical</CModalTitle>
              </CModalHeader>
              <CModalBody>
                <UpdateChemicalComponent onclick={()=> this.setState({showEdit: false})} toBeUpdatedItem={this.state.toBeUpdatedId}/>
              </CModalBody>
            </CModal>
            ...
            >
              <CModalHeader>
                <CModalTitle>Chemical Item Details</CModalTitle>
              </CModalHeader>
              <CModalBody>
                <GetChemicalComponent onclick={()=> this.setState({showDetail: false})} fetchedItem={this.props.fetchedItem}/>
              </CModalBody>
            </CModal>
...
const mapDispatchToProps2  = {
  ...
  const CreateChemicalComponent: React.FC<any> = connect(mapStateToProps2,mapDispatchToProps2)((props: any) => {
    ...
    const onSubmit = (data: IState2) => {
      ...
      props.createItem(createChemical,inputData);
      props.onclick();
    };
...
interface IState3 {
  ...
  const mapDispatchToProps3  = {
    ...
    const UpdateChemicalComponent: React.FC<any> = connect(mapStateToProps3,mapDispatchToProps3)((props: any) => {
      //console.log('rendered with props:',props);
      ...
      const onSubmit = (data: IState3) => {
        ...
        props.updateItem(updateChemical,inputData);
        props.onclick();
      };
...
const mapDispatchToProps4  = {
  ...
  const GetChemicalComponent: React.FC<any> = connect(mapStateToProps4,mapDispatchToProps4)((props: any) => {
    //console.log('rendered with props:',props);
